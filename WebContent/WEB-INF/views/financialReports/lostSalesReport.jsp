...
<body>
	<form:form action="lostSalesReportAction" method="post"
		...
		<h3>
			Lost Sales Report Category Wise for
			<c:out value="${mode}" />
		</h3>
		...
		<h3>
			Lost Sales Report Variant Wise for
			<c:out value="${mode}" />
		</h3>
		<c:if test="${!empty vproducts}">
			<div class="scrollingtable">
				<div>
					<div>
						<table>
							<thead>
								<tr>
									...
									<th><div label="Variant Type"></div></th>
									<th><div label="Quantity"></div></th>
									<th><div label="Amount"></div></th>
									<!-- <th><div label="Date"></div></th> -->
								</tr>
							...
							<tbody>
								<c:forEach items="${vproducts}" var="product">
									<tr>
										<c:choose>
											<c:when test="${product.quantity>0}">
												<td><c:out value="${product.productName}" /></td>
												<td><c:out value="${product.variantName}" /></td>
												<td><c:out value="${product.variantType}" /></td>
												<td align="right"><c:if test="${product.quantity>0}">
														<c:out value="${product.quantity}" />
													</c:if></td>
												<td align="right"><c:if test="${product.rate>0 }">
